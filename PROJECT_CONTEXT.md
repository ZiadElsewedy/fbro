...
│   ├── enums/                # user_role · approval_status · task_* · task_assignment_type · template_repeat_mode · recurrence_frequency · notification_type · schedule_day · schedule_shift · leave_type · swap_status · broadcast_audience · broadcast_category · broadcast_recurrence · case_category · case_recipient · case_privacy · case_status · request_type · request_status · event_type · event_status · event_phase
...
└── features/
    ...
    ├── community/            # Community Hub / DROP Events (2026-07-08) — the flagship event feature. An **event is its own operational workspace**, not a calendar row: a single `events/{id}` doc embeds identity + every section (timeline `EventMilestone` by `EventPhase` · team `EventAssignment` · tasks `EventTask` · `EventInventoryItem` · `EventLogisticsItem` · `EventBudgetLine` · `EventAnnouncement` · after-event `EventOutcome`). Plain-immutable `EventEntity` (no codegen, the `BroadcastScheduleEntity` pattern) + `EventModel` (deep nested (de)serialization) + `EventRepository(+Impl)`/`EventRemoteDataSource` (Firestore + Storage hero image). Pure `event_readiness.dart` (0–100 score + blockers/warnings/wins — the intelligence layer) + `event_ordering.dart` (live → soonest-upcoming → archive). App-wide `CommunityHubCubit` (role-scoped realtime stream + create, repo-direct) + per-event `EventWorkspaceCubit` (streams the doc, **one write path** `copyWith → updateEvent`; built via `AppDependencies.createEventWorkspaceCubit`). UI: `community_hub_screen` (spotlight rail + archive), the cinematic **`event_workspace_screen`** (collapsing hero → chapters Overview→Readiness→Timeline→Team→Tasks→Inventory→Logistics→Budget→Communication→After; a live event floats a command center, a completed one becomes an archive), `create_event_screen`, + `event_format.dart`/widgets. Enums `event_type`/`event_status`/`event_phase`. Editing gated to admin + manager (mirrors rules); employees read-only. Routes `/community`, `/community/create`, `/event/:eventId`. **Client-only (no Cloud Functions).**
