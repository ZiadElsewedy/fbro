...
| **Activity timeline (schema / logic)**    | `lib/features/task/domain/entities/activity_entry.dart` (freezed: status/actorId/actorName/at/note) + `task_entity.dart` (`activityLog` field) + `task_model.dart` (`_activityLogFromList`/`_activityLogToList`). Entries are appended **inline** in each status-changing `TaskCubit` method (single atomic write; no `_appendActivity` helper). This **is** the spec's event-based task timeline — rendered dynamically (supports missing/optional steps + rework loops) via `TimelineTile` + `activity_format.dart` |
...
| **Admin all-branch swap visibility**      | `ScheduleRepository.getAllSwaps()` (+ datasource + impl) → `ShiftSwapCubit.pendingSwaps()` (one-shot, non-emitting, for the Admin Home count) **and** `ShiftSwapCubit.loadAll()`/`SwapScope.all` (the list state for the admin **Swap Requests tab** on `ScheduleManagementScreen`) |
| **Schedule orphan / broken-reference handling** | `schedule_helpers.dart` (`isOrphanAssignment`/`shortUid`) → `manager_schedule_view.dart` (`_orphanUids` banner + `_brokenChip` + `_resolveOrphan` remove/reassign). A slot uid that isn't a current branch member is flagged, never shown as a fake "Unknown" name. Tested in `test/schedule_helpers_test.dart` |
| **Schedule screens (admin/manager/employee)** | `lib/features/schedule/presentation/pages/` (`schedule_management_screen` admin — **2-tab: Schedule + all-branches Swap Requests** · `branch_schedule_screen` manager — 2-tab: Schedule + branch Swap Requests · `my_schedule_screen` employee — My Week + Swaps; past slots show "Past" not a Swap action) → shared `widgets/manager_schedule_view.dart` · `swap_view.dart` (`SwapListView` has `showBranch` for the admin queue) · `schedule_helpers.dart` |
