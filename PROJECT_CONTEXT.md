...
| **Admin Home (command center)**           | `lib/features/admin/presentation/pages/admin_dashboard_screen.dart` — greeting · hero (most-urgent insight) · `DashboardMetricCard` grid · `ActionCard` quick actions · pending-approvals preview · recent-activity feed (`TimelineTile`) · Manage grid. Reads `StatisticsCubit` + `TaskCubit` (all-branches stream, for activity/overdue) + `AdminUsersCubit.pendingUsers()` |
...
| **Vertical timeline row (shared)**        | `lib/core/widgets/timeline_tile.dart` (`TimelineTile`) — used by the Task Details activity timeline + the admin recent-activity feed |
