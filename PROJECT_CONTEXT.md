...
│   ├── enums/                # user_role · approval_status · task_* · task_assignment_type · template_repeat_mode · recurrence_frequency · notification_type · schedule_day · schedule_shift · swap_status · broadcast_audience · broadcast_category · broadcast_recurrence
...
- **Self-service contact & payroll (2026-07-02):** `ProfileEntity` carries
  `address` / `emergencyContact` / `paymentNumber` (all stored on the same
  `users/{uid}` doc; `paymentNumber` threaded through
  `editMap → datasource → repo → UpdateProfile → ProfileCubit.save`). **Edit
  Profile** exposes validated Contact details (phone · address · emergency) +
  Salary payment number sections, so an employee keeps their own record current
  without an admin relay; the Profile page displays the rows. The admin-only
  salary fields (amount/type/method) are NOT part of the profile contract —
  they're frozen by the `users` self-update rule.
...
- **`admin`** owns user administration over `users/{uid}` via its own
  ...
  **non-privileged** info — `displayName` (mirrored to legacy `fullName`),
  ...
  writes them too).
  **Compensation record (2026-07-02):** `UserEntity`/`UserModel` carry
  `salaryAmount` / `salaryType` (`monthly`/`weekly`/`daily`) / `paymentMethod`
  (`cash`/`bank`/`wallet`/`instapay`) / `paymentNumber` (the number salary is
  transferred to) — all excluded from `UserModel.toMap`.
