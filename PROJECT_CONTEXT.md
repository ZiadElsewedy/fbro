...
admins override any branch; a **Reports Center** (Reports / Escalation System,
2026-07-03): any employee files a categorized, severity-rated report, routes it
to their manager and/or admin (optionally **confidential / anonymous** — the
reporter's identity is isolated in a private subdoc), and the recipient
acknowledges → works → resolves it, with an audit timeline, an internal
discussion thread, and attachments. All dressed in the **DROP THE SHOP** design system —
...
│   ├── enums/                # user_role · approval_status · task_* · task_assignment_type · template_repeat_mode · recurrence_frequency · notification_type · schedule_day · schedule_shift · swap_status · broadcast_audience · broadcast_category · broadcast_recurrence · report_category · report_recipient · report_privacy · report_severity · report_status
...
└── features/
    ...
    ├── schedule/             # Weekly schedule + shift swaps (Phase 7) — full slice + ScheduleCubit & ShiftSwapCubit; weekly_schedules + shift_swaps. **Schedule 3.0 (Phase 2, 2026-07-02):** per-person `AssignmentChip`s in the grid cells (desktop drag-to-move via `ScheduleCubit.move` — assign-then-remove in one busy cycle; right-click/long-press menu: move-to-opposite [double-booking-gated] · remove); pure `presentation/schedule_insights.dart` (open · one-person · double-booked facts, NEVER quotas) drives a clickable insight strip that highlights/dims grid cells + per-chip red conflict dots; swap queue = strip chip (floating footer + coverage %-bar card removed). **Schedule 3.1 (2026-07-02, owner-extended drag scope):** desktop **drag-to-switch** — a chip dropped ON another person's chip trades their slots via `ScheduleCubit.exchange` (assign-both-first-then-release, one busy cycle; self-swap/same-slot = no-op); `AssignmentChip` is itself a `DragTarget` (⇄ + primary ring when targeted) that wins the hit test over its host `ShiftCell`, so person-drop = switch, empty-cell-drop = move (`onSwapChip` threaded grid→view); schedule empty states are brand-led `DropEmptyState`s + a quiet `DropLogo` signs the grid-hint row (tested in `test/schedule_exchange_test.dart`). **Schedule 4.0 (2026-07-02):** (a) **crowded cells** — ≤4 people render all chips, >4 collapse to 3 + a tappable "+N more" → the shift panel; (b) **mobile actions** — touch long-press on a chip opens `chip_action_sheet.dart` (Move via mini week map · Switch via (person, slot) picker → trade preview → confirm · Remove); desktop right-click adds "Switch shifts with…" → the same flow; (c) **undo** — `ScheduleCubit` records the inverse of every move/exchange/remove (`undoWindow` 5s, `canUndo`/`undoLast`, single-use, any newer mutation invalidates it) surfaced as an UNDO snackbar in `manager_schedule_view`; (d) **validation** — pure `domain/move_validation.dart` (`checkMove`/`checkExchange` → user-facing reason or null: double-booking blocked, exchange positions gated by the branch `SwapPolicy`; `wouldEmptySlot` → confirm-not-block, facts-never-quotas) — every grid edit path funnels through validated helpers in `manager_schedule_view`; (e) `ScheduleCubit.load` is **silent on a same-scope reload** (no loading flash on revisit; a branch/week change still shows the loader). Tests: `move_validation_test` · `schedule_undo_test` · `schedule_silent_reload_test` · `chip_action_sheet_test`
    ...
    ├── reports/              # Reports Center (Reports / Escalation System, 2026-07-03; **simplified 2026-07-04** — escalation messages, NOT tasks) — full slice + app-wide ReportCubit. Branch-scoped reports with **5 categories** (Sales/Inventory/Staff/Security/Operations), recipient, **privacy (normal/confidential)**, severity, and a **4-step lifecycle New→Under Review→Waiting Reply→Resolved** (no ownership/assignment). **Rule-enforced privacy split** — the report doc carries NO creator uid; the reporter identity lives in the private subdoc `reports/{id}/reporter/identity` (owner + admin only, mirrors the compensation subdoc), and `reporterDisplayName` rides the doc only for a `normal` report. **Conversation model:** `activityLog` is the thread (opening report + `status:'comment'` replies + status markers; reuses the task `ActivityEntry` + `TaskAttachment`). Pure `report_urgency.dart` client-side SLA (no cron). Notifications are **server-side** (`onReportCreated` / `onReportUpdated` Cloud Functions — a manager can't read a confidential reporter to notify them). Screens: `reports_center_screen` (role-scoped list; admin can't file), `create_report_screen` (fast flow; manager→admin-locked), `report_details_screen` (**premium chat/support conversation** — message-first card + `report_thread` bubbles + recipient status bar + pinned composer). Routes `/reports`, `/reports/create`, `/report/:reportId`
...
> for the realtime feed stream. `ReportCubit` (Reports Center) is the same
> hybrid: `CreateReport`/`UpdateReport`/`UploadReportAttachment` use cases for
> writes, **`ReportRepository` directly** for the role-scoped list (admin: all
> stream · manager: branch stream + own reports · employee: own reports via the
> `reporter` collectionGroup), reusing **`BranchRepository`** (branch names) +
> **`GetUsersByBranch`** (assignee directory). All app-wide cubits are provided
> in `main.dart`
> (`auth`/`profile`/`task`/`branch`/`adminUsers`/`statistics`/`schedule`/`shiftSwap`/`branchOperations`/`broadcast`/`notification`/`report`).
...
| **Recurring shift-task templates (daily/weekly generated instances)** | `core/enums/template_repeat_mode.dart` + `recurring_task_template_entity.dart`/`recurring_task_template_model.dart` (collection `recurringTaskTemplates`) + `TaskRepository.{get,create,update,delete}RecurringTemplate` + `TaskCubit.createRecurringShiftTemplate`/`_materializeTodayInstance` (client-side "today" instance, deterministic id) + Cloud Function `generateShiftTaskInstances` (`functions/index.js`, `onSchedule` every 24h, same deterministic id `rt_{templateId}_{yyyy-MM-dd}` — the existence check is the whole duplicate-prevention guarantee) + UI `recurring_shift_task_sheets.dart` (`showManageRecurringShiftTasksSheet`, wired from `BranchOperationsScreen`'s app-bar action) |
