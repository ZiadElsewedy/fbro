...
| **Task media (images + videos)** | Model: `domain/entities/task_attachment.dart` (`TaskAttachment` + `AttachmentLimits`) + `core/enums/attachment_type.dart`; attached to `ActivityEntry.attachments[]`. Pick: `presentation/widgets/attachment_picker.dart` (`AttachmentPickerField`). View: `attachment_gallery.dart` + `attachment_viewer.dart` (fullscreen zoom/`video_player`). Resolve/format: `presentation/attachment_format.dart` (`attachmentsForEvent` / `latestAttachments`, legacy-proof back-compat). Upload: `UploadTaskAttachment` use case. Tested in `test/task_attachment_test.dart` |
