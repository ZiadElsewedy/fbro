...
**Open → In Discussion → Waiting Response → Closed** until resolution; and
**Operations Requests** (2026-07-08): an employee asks their own-branch manager
for a yes/no approval (admin has global visibility/escape hatches), with a lean
**Create → Pending → Approved / Rejected** lifecycle, one message field,
optional attachments, server-written timeline events, admin soft delete, and
admin reopen. All dressed in the **DROP THE SHOP** design system —
...
│   ├── enums/                # user_role · approval_status · task_* · task_assignment_type · template_repeat_mode · recurrence_frequency · notification_type · schedule_day · schedule_shift · leave_type · swap_status · broadcast_audience · broadcast_category · broadcast_recurrence · case_category · case_recipient · case_privacy · case_status · request_type · request_status · attendance_status · attendance_source · attendance_location_policy · attendance_correction_kind
│   ├── extensions/           # context_extensions (currentUser/currentRole) · firestore_extensions (Map.date — Timestamp→DateTime)
│   ├── services/             # notification_service.dart (FCM foundation, Phase 6; gated on `supportsPushNotifications` — desktop skips permission+registration; Apple platforms check `getAPNSToken()` before `getToken()`, 2026-07-02; **Notifications V2 2026-07-10:** `onForeground` now carries the push `data` for an actionable in-app snackbar, and both tap surfaces route through the single pure resolver `features/notifications/domain/notification_deep_link.dart` (`resolveNotificationRoute` — role-aware, `null` = guarded no-op → inbox fallback, never crashes; server `onNotificationCreated` mirrors the route strings + forwards `taskId·caseId·requestId·broadcastId·swapId`). Android manifest declares `POST_NOTIFICATIONS`. iOS push still unconfigured — see `docs/design/NOTIFICATIONS_V2.md`)
...
└── features/
    ...
    ├── task/                 # Task feature — data/domain + use cases + TaskCubit + functional role screens (Phase 3–4); realtime streams + templates (Stabilization); Phase 9 — multi-assignee + checklist + redesigned cards; Workflow Upgrade — RecurrenceConfig + ActivityEntry + TaskDetailsScreen + MyTasksScreen redesign; presentation/activity_format.dart (shared timeline label/colour/time helpers); Premium task UX slice (2026-06-25) — admin/manager reference images (`TaskEntity.referenceAttachments`, reused `AttachmentPickerField`) + a redesigned, **de-flashed** signal-driven `TaskCard` (flat `TaskSurface` primitive — no glow/gradient/pulse; status pill · High-only priority · branch/due/refs chips · thin checklist bar); **Shift Assignment feature (2026-07-01)** — `TaskEntity.assignmentType` (individual/team/shift) + `domain/task_access.dart` (`canUserAccessTask`) + merged per-shift task streams in `TaskCubit` + recurring shift-task **templates → generated instances** (`RecurringTaskTemplateEntity`, `recurring_shift_task_sheets.dart`, Cloud Function `generateShiftTaskInstances`) — see "Task chain" narrative for the full design; **Task Scheduling V2 (2026-07-08)** — additive `TaskEntity.startsAt` (the due side stays the existing `deadline`, aliased `dueAt`; no migration) + pure `domain/task_schedule.dart` (`TaskSchedulePhase` Scheduled/Active/Due-soon/Overdue/Done **derived** from times + lifecycle — **not** a new `TaskStatus`; `shiftDefaultSchedule(date, shift)`; `dueSoonCount`). The create form's Deadline section became a **Schedule** section (`_ScheduleField`/`_ScheduleBanner` in `form_primitives.dart`): Start + Due **date & time**, pre-filled from the assigned shift's `ShiftHours.standard` as a *smart default* with a "Suggested from … / Custom schedule · Reset to shift" banner (never locked). Admin dashboard Today strip gains **Due soon**; cards/details show the start→due window. **Refinement:** smart defaults extend to individual/team via `TaskCubit.resolveAssigneeShift` (branch roster → pure `assigneeShiftFit`: unanimous→suggest, mixed→a Morning/Night/Custom chooser, none→manual); blocking due-before-start validation + non-blocking outside-shift-hours warning; overnight windows; the banner keeps the **original** shift after edits ("Originally: …"); `scheduledDuration`/`formatScheduleDuration` show estimated duration in the form + Task Details (which also shows phase · Completed · Est. duration)
    ├── branch/               # Branch feature — data/domain + BranchCubit + branch management (Phase 5)
    ├── admin/                # Admin module — user-admin data/domain + AdminUsersCubit + dashboard/managers/employees/approvals (Phase 5). **Admin Home = DROP Design System V2 command center (2026-07-08, premium-polished 2026-07-09):** a progressive-disclosure ladder built entirely from reusable V2 primitives — **Hero** (`PageHero`: greeting + a live **contextual system mood** — pure `admin/presentation/dashboard_mood.dart`: "2 tasks need your attention" / "Everything's running smoothly" / "Quiet morning", escalating calm→attention→busy — with a breathing "system-alive" pulse dot, the scope `branches · employees · running` quiet beneath, **one** *Create Task* CTA · quiet Sync/⌘K) → **Needs attention** (the dominant layer: `AttentionTile`s for Pending review · Overdue · Unassigned · Sent-back · Swaps, the single most-urgent carrying the living border; each drills to a filtered task view via `FilteredTasksScreen`; a *cleared* tile rewards the healthy state with a ✓ + a positive line via `clearedMessage`, never a bare "0") → **Today** (`StatStrip`: completed today · running · delayed · approval rate) → **Recent activity** (`RecentActivityFeed` of clean `ActivityCard`s, **no filter chips**, a softly-breathing "all clear" empty state + skeleton loading, tap → the shared `showTaskPreviewSheet` preview → optional full details) → **Operations digest** (pending requests · active cases · schedule coverage) → Quick actions / Manage / Branch pulse. Every section is a scoped `BlocSelector` (live, no manual refresh); reduced-motion + `PageStorageKey` preserved. EmployeeCard + employee_metrics (computeEmployeeMetrics — per-employee perf from the task stream)
    ...
    ├── schedule/             # Weekly schedule + shift swaps (Phase 7) — full slice with `WeeklyScheduleEntity`, `ScheduleCubit`, and `ShiftSwapCubit`; Firestore collections `weekly_schedules` + `shift_swaps` + `shift_templates`. The week doc carries assignments, multi-line `dayNotes`, day-level `leave`, per-slot `shiftHours` overrides, and (Pillar 5) an optional `shiftPlan` snapshot (`ShiftHours`; all read through `WeeklyScheduleEntity.hoursFor`, which resolves **per-slot override → the week's `shiftPlan` snapshot → `ShiftHours.standard`** — a legacy week with no snapshot resolves standard, unchanged). Manager/admin use the weekly schedule grid + day-details sheet + advisory insights + a rule-based **Schedule Health analyzer** + a premium printable **Final View**; employee uses the owner-frozen premium My Week hero/week rows + tap-to-open shift sheet. **Schedule Health (Schedule V2 · Pillar 3)** is a modular domain engine in `domain/health/`: `ScheduleAnalysis.of(...)` reduces the roster to shared one-pass signals, then five independent pure rules (`CoverageRule`/`WorkloadRule`/`FairnessRule`/`RestRule`/`ConflictRule`, each a `ScheduleRule.evaluate(analysis)`) are folded by `ScheduleHealthAnalyzer` into a `ScheduleHealthReport` (overall score/severity + per-category `ScheduleRuleResult`s + findings). It is **advisory, never a gate**. `domain/schedule_health.dart` is the backward-compatible facade — the legacy `computeScheduleHealth()` delegates to the analyzer and projects the exact original score/findings. **Desktop layout (calm/grid-first, 2026-07-10 UX polish — presentation-only):** the grid is the hero. The **global** health / insights / legend render in a lightened, borderless below-grid **`ScheduleOverviewSurface`** (Health reduced-weight score + category breakdown + ≤3 one-line findings whose fix reveals on tap · Insights = a few attention-fact lines · Legend **collapsed by default**, near-invisible). The right **inspector rail** (`schedule_inspector_drawer.dart` — weekly summary + team list + per-employee contextual detail) is a **collapsible + resizable contextual workspace**: a toolbar `view_sidebar` toggle + a draggable splitter (280–460px), **open state + width remembered for the session** (`_InspectorPrefs`, in-session only — no local-prefs store), **hidden by default on the 1024–1600 desktop tier / open on ultrawide**; collapsing it hands the width back to the grid (`ClipRect`+`OverflowBox`+`AnimatedContainer`, reduced-motion honoured). Touch widths keep the stacked column + bottom sheets. **Final View (Schedule V2 · Pillar 4)** is a **dedicated read-only presentation surface** `widgets/final_schedule_sheet.dart` (`FinalScheduleSheet`) — a premium printable spreadsheet (employee rows × day columns, `M`/`N`/`OFF`/`LEAVE`/`VAC` tokens, header + notes row + legend), hosted by `pages/schedule_final_view.dart` which keeps only the launch route (`showScheduleFinalView`) + the PNG export (`RepaintBoundary`→`toImage`→Downloads). **Shift Templates (Schedule V2 · Pillar 5)** are the configurable, per-branch source those hours resolve from: collection `shift_templates/{branch}__{role}` (`ShiftTemplate` = name + role[morning/weekdayNight/weekendNight/custom] + `ShiftHours`; `ShiftTemplateSet` derives the `ShiftPlan`), a `ShiftTemplateRepository(+Impl+datasource)` + `ShiftTemplateCubit`. A new week **snapshots** the branch's plan onto its `shiftPlan` (history-safe by construction); managers edit hours with a 3-way scope (`ShiftHoursScope`) via `ScheduleCubit.applyShiftHours` — This week (per-slot override) · Future (updates the template) · Global (template + `ScheduleRepository.restampShiftPlan` onto current/future weeks). UI: `shift_templates_sheet.dart` + `shift_hours_scope_dialog.dart` off the day sheet. Slot timing is centralized in `domain/shift_window.dart` (`startOf`/`endOf`/`phaseOf`/`nightSpillEnd`); the Sunday small-hours Sat→Sun seam uses `ScheduleCubit.previousSaturdayNight` for previous-week crew context. Swap validation remains client/domain plus callable `approveSwap` for the final manager-approved exchange.
    ...
    ├── requests/             # Operations Requests (2026-07-08) — employee approval requests, not tickets. Full slice: `RequestEntity`/`RequestModel`/`RequestEvent`, `RequestRepository(+Impl)` + `RequestRemoteDataSource`, write use cases (`CreateRequest`, `ChangeRequestStatus`, `AddRequestComment`, `UploadRequestAttachment`), app-wide `RequestsListCubit` (role-scoped streams + employee-only create, branch names), per-request `RequestDetailCubit` (doc + events stream, approve/reject, admin reopen/soft-delete), pure `request_access.dart`/`request_ordering.dart`/`request_metrics.dart`/`request_thread.dart`, and UI pages `requests_screen`, `create_request_screen`, `request_detail_screen` + `request_card`/`request_timeline`/`request_composer`. Flow: employee-only create → Pending → Approved/Rejected; admin can soft-delete (`deletedAt`) and reopen; server functions write `REQ-######`, timeline events, and notifications. Routes `/requests`, `/requests/create`, `/request/:requestId`.
    # community/ (Community Hub / DROP Events) was REMOVED 2026-07-15 (owner request) — the feature, its 4 tests, the event_type/event_status/event_phase enums, eventsCollection, all routes/sidebar-nav/DI wiring, and the events/{id} Firestore + events/** Storage rules were all deleted. Live Firestore data left untouched.
    ├── attendance/           # Attendance — GPS clock in/out + corrections + admin oversight (Phase 1 2026-07-11 · Phase 2 2026-07-14 · Phase 3 2026-07-15). **Full-stack: data foundation + GPS + Employee & Admin UI.** A record is `attendance/{uid}_{yyyyMMdd}_{shift}` (deterministic id → idempotent, offline-safe clock-in; see `domain/attendance_id.dart`); the append-only audit trail lives in the `attendance/{id}/events` subcollection and is **written ONLY by the Admin SDK** (server-authoritative — clients can't forge audit). Pure domain: `AttendanceCalculator` (**the single source of worked/late/early/overtime/break minutes** — persisted as a snapshot only at clock-out / correction-approve), `AttendanceValidation` (clock + `checkCorrection` gates), `AttendanceConfig` (grace/geofence/photo policy + module dark-switch), `AttendanceBreak`/`AttendanceLocation`/`AttendanceResolution` value objects, `AttendanceStats` analytics, `AttendanceService` (config/dark-switch seam). `AttendanceEntity` + `AttendanceEvent` (freezed) + `AttendanceCorrectionEntity` (freezed, reuses `RequestStatus`). Employee **`AttendanceCubit`** drives the whole surface from ONE realtime history stream (`AttendanceFeed` = records + offline/pending-write metadata → exposes today/session/loading/**syncing**/**offline**/clock-availability/validation-errors), resolving today's shift via the existing `ScheduleRepository` seam (no re-derivation); clock actions write ONLY the record (audit is derived server-side). **Corrections** (`attendance_corrections/{id}`, Phase 2): a first-class `Pending → Approved/Rejected` approval object — employee files (`RequestCorrection`), reviewer decides (`DecideCorrection`, which computes the corrected snapshot via `AttendanceCalculator`); the approved resolution is applied to the record + the audit event written **server-side** by `onAttendanceCorrectionWritten`. Cloud Functions `onAttendanceWritten` (derive audit by diffing), `onAttendanceCorrectionWritten` (correction lifecycle + apply + notify), `autoCloseAttendance` (scheduled → never-clocked-out sessions to `pendingReview`) — all mirror `onRequest*` and reuse the notification pipeline. Enums `attendance_status`/`attendance_source`/`attendance_location_policy`/`attendance_correction_kind`. Rules: employee reads/clocks own · manager own-branch · admin all; corrections forbid self-approval; audit events client-read-only. **Phase 3 (2026-07-15) — GPS clock-in/out (full-stack):** clock in/out are GPS-verified via `geolocator` (`AttendanceLocationService`/`GeolocatorLocationService`) + pure `attendance_gps.dart` (`gpsDistanceMeters` Haversine + `AttendanceVerification`=distance/within-radius/accuracy-ok/verified, snapshotting the branch radius+accuracy floor). Branch `BranchGeofence` (lat/lng/radius/minGpsAccuracy) + `BranchRepository.setGeofence`. Record stores `clockInVerification`+`clockOutVerification` SEPARATELY; clock TIMES are server timestamps (`effectiveClockIn` covers the live timer until sync). `checkClockIn`=eligibility, `checkGpsFix`=GPS gate (rejects no-shift/service-off/permission-denied/low-accuracy/outside-radius). **Break flow REMOVED** for the MVP — `AttendanceBreak`/`breaks`/calculator-netting kept as dormant extension points. **UI (2026-07-15):** **Employee** `attendance_screen.dart` (route `/attendance`) — Today's Shift → GPS-gated Clock In → live `HH:MM:SS` Working → Today's Summary; a **state-driven GPS card** off a live `previewLocation()` (Checking/At-branch/Outside/Permission/Off); clock-out records verification but is **never GPS-blocked**. **Admin** `admin_attendance_screen.dart` (route `/admin/attendance`, admin sidebar) — the **schedule × attendance board**: pure tested `attendance_board.dart` `computeAttendanceBoard(roster, records, now, config)` derives Not-started→Late→Absent (by time) + Working/Completed/On-leave/Needs-review; `AttendanceAdminCubit` fuses roster (`getSchedule`+`employeesFor`+`ShiftWindow`+`GetUsersByBranch`) with live `watchBranchDay` + `watchBranchPendingCorrections`; branch picker · filterable Working/Late/Absent KPIs · details sheet · corrections approve/reject (`DecideCorrection`) · GPS-area shortcut. **Admin geofence editor** `branch_geofence_editor_screen.dart` (per-branch "Set GPS area", `Geolocator` current-location, `BranchGeofence.validateInput` → `BranchCubit.setGeofence`). **The Admin cubit+screen are branch-scoped so a future MANAGER view reuses them pinned to own branch (descoped for V1).**
...
> for the realtime feed stream. **Requests** splits into an app-wide
> `RequestsListCubit` (role-scoped streams + employee-only create, with
> `BranchRepository` for branch names) and per-request `RequestDetailCubit`
> instances (built on demand by `AppDependencies.createRequestDetailCubit`) that
> stream the request doc + `requests/{id}/events` and own comment/decision/admin
> reopen/delete actions through the request use cases/repository. **Case
> Management** splits into two cubits:
...
> (`auth`/`profile`/`task`/`branch`/`adminUsers`/`statistics`/`schedule`/`shiftSwap`/`branchOperations`/`broadcast`/`notification`/`caseList`/`requestsList`).
...
TaskCubit._transition → TaskRepository.transitionTask  (txn: verify predecessor status · append ActivityEntry to SERVER log · bump version)
TaskCubit._spawnNextRecurrence  (post-commit on approve — only the winning reviewer spawns the next recurring task)
        ↓
...
- **Every status transition is a single atomic Firestore *transaction*** via `TaskRepository.transitionTask` (2026-07-11 P0 hardening). It re-reads the doc, verifies the current `status` is a legal predecessor (the state machine, now the `from:` set passed to `TaskCubit._transition` — replaces the old client-only `_canTransition`), appends the new `ActivityEntry` to the **server's** current `activityLog` (never a stale client array), merges only the changed fields (status + its per-transition timestamp `startedAt`/`submittedAt`/`approvedAt`/`rejectedAt` + review fields), and bumps an optimistic-concurrency `version`. This closes the concurrent-reviewer race — a lost activity-log entry and a phantom recurrence double-spawn: a stale transition raises a `ConflictFailure` (surfaced as a benign "refreshed" notice) and the realtime stream delivers the true state; the recurrence spawn now runs **post-commit**, so only the reviewer that won the transaction creates the next instance. Plain **`updateTask` is content-only** — the datasource strips `_transitionOwnedFields` (status, log, review fields, `version`, `archivedAt`), so a stale content edit / checklist tick can never clobber the log or regress a lifecycle field. The in-instance `_mutating` flag still blocks a local double-tap. WHO may write is enforced in `firestore.rules` (`tasks/{taskId}`): admin all branches, manager own branch, employee own assigned tasks with **limited writes** (may advance status up to `waitingReview` / add notes / proof but may not reassign, change branch, approve/reject, forge review attribution, or **shrink/rewrite the activity log** — 2026-07-11 P1-5). Proof images upload to Storage under `tasks/{taskId}/attachments/`.
...
| **Task reads/writes / media upload** | `lib/features/task/data/datasources/task_remote_datasource.dart` (Firestore; the media upload delegates to the shared **`core/media/media_upload_service.dart`** — `MediaUploadService`, the **single Storage seam** for task/case/request attachments: `tasks/{id}/attachments/{id}.<ext>`, unique-id · content-type · `Cache-Control` · 180s timeout · error translation, all in one place instead of the former per-datasource copies) |
...
| **Task media (images + videos)** | Model: `domain/entities/task_attachment.dart` (`TaskAttachment` incl. `durationMs` + `AttachmentLimits`) + `core/enums/attachment_type.dart`; attached to `ActivityEntry.attachments[]`. Pick: `presentation/widgets/attachment_picker.dart` (`AttachmentPickerField`, captures video duration via `video_player`; **mobile pre-upload editing** — crop/rotate/flip via `core/media/media_processing.dart` `editImage` [`image_cropper`] after a camera capture + a tap-to-edit affordance on each image thumbnail, and picked videos transcoded by `compressVideo` [`video_compress`] behind a cancellable dialog — both gated on `supportsImageEditing`/`supportsVideoCompression`, so desktop/web upload untouched). View: `attachment_gallery.dart` (compact wrap **or** `columns` grid + `showDuration`) + `attachment_viewer.dart` (fullscreen zoom/`video_player`; play/pause-only rebuilds + `cacheWidth` decode cap) + `video_thumbnail_image.dart` (cached real frames). Resolve/format: `presentation/attachment_format.dart` (`attachmentsForEvent` / `latestAttachments` / `attachmentSummary` / `formatVideoDuration`, legacy-proof back-compat). Upload: `UploadTaskAttachment` → the shared `MediaUploadService`; `PickedAttachment` now lives in `core/media`, and submission uploads run through `core/utils/concurrent.dart` `mapPooled` (concurrency cap 3, fixed-denominator progress). Tested in `test/task_attachment_test.dart` + `test/concurrent_test.dart` |
...
| **Submission loading UX / progress** | Lives on the cubit: `TaskState.loaded` carries `isSubmitting` + `submissionProgress` (`presentation/submission_progress.dart`: Preparing→Uploading→Finalizing, with bytes→%/MB), preserved on every emit incl. the stream. `completeAndSubmit` throttles progress emits (whole-percent); byte progress aggregated from each upload's Storage `snapshotEvents`. The Task Details screen renders one state-driven `submission_loading_overlay.dart` (fullscreen, interaction-blocking, `PopScope` blocks back). Only `completeAndSubmit` sets `isSubmitting`. **Cancellation (2026-07-11):** the overlay's **Cancel** button → `TaskCubit.cancelSubmission()` aborts every in-flight upload via an `UploadCanceller` (threaded use case→repo→datasource→service like `onProgress`; throws `UploadCancelledException` → **quiet** UI restore with the media kept), hidden during `finalizing` so the Firestore write finishes (no mid-commit orphan). **Retry:** a per-task `_uploadedCache` (file path → result) makes a re-submit after a partial failure/cancel re-upload **only what didn't already succeed**. **Analytics:** `media.upload_started/completed/failed/cancelled` audit events (via the existing `EventTrackingService`) carry `durationMs` · byte/count · `compressionRatio` (from `PickedAttachment.originalBytes`). Video thumbnails are **local** (`video_thumbnail_image.dart`, view-time, LRU cache) — no server posters. Tested: `test/submission_progress_test.dart` + `test/upload_canceller_test.dart` |
...
| **Task workflow logic / status transitions / state** | `lib/features/task/presentation/cubit/task_cubit.dart` (`_transition` → `TaskRepository.transitionTask`, txn-guarded predecessor + server-log append + `version`) + `task_state.dart` |
...
| **Activity timeline (schema / logic)**    | `lib/features/task/domain/entities/activity_entry.dart` (freezed: status/actorId/actorName/at/note/**attachments**) + `task_entity.dart` (`activityLog` field) + `task_model.dart` (`_activityLogFromList`/`_activityLogToList`, incl. `_attachmentsFromList`). Entries are appended atomically in each status-changing `TaskCubit` method via `TaskRepository.transitionTask` (append to the server's current log inside a transaction — 2026-07-11). This **is** the spec's event-based task timeline — rendered newest-first as rich `_EventCard`s (status badge, actor, note, **`AttachmentGallery`**) via `activity_format.dart` + `attachment_format.dart` |
...
- **New surfaces compose the Design System V2 primitives** (`PageHero` ·
  `AttentionTile` · `StatStrip` · `ActivityCard`) — see
  `docs/design/DESIGN_SYSTEM_V2.md`. Calm through **hierarchy**, not reduction;
  one primary CTA per screen; every widget carries a Semantics label, ≥44px
  targets, honours reduced motion, and renders collections lazily/capped.
  **`core/widgets` must never import a feature** — apply feature-specific motion
  (e.g. the `LiveStatusBorder` orbit) at the call site via the exposed hook
  (`AttentionTile.radius`), not inside the primitive.
...
- **Enforcement** lives in `firestore.rules`: reusable `isAdmin()`/`isManager()`/
  ...
  **`task_templates/{id}` (Stabilization)** are manager/admin-readable reusable
  ...
  unaffected. **`requests/{id}` (Operations Requests)** are branch-scoped
  approvals: create = authenticated employee/requester in their own branch;
  read = requester, admin, or own-branch manager; update = admin or own-branch
  manager for approve/reject/reopen, with admin-only soft delete via
  `deletedAt`; hard delete is denied. `requests/{id}/events` is append-only for
  participants; server functions write lifecycle events, `REQ-######`, and
  notifications. The `users` self-update rule also freezes the admin-set `position`.
